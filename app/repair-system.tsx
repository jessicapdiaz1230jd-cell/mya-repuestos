...
export default function RepairSystem({ accessToken = "", user = { name: "Administrador", email: "", role: "admin" }, onLogout }: { accessToken?: string; user?: CurrentUser; onLogout?: () => void }) {
  ...
  return <div className="app-shell" style={themeStyle}>
    ...
    <OrderDialog accessToken={accessToken} open={detailOpen} setOpen={setDetailOpen} detail={selected} saving={saving} onSubmit={updateOrder} />
  </div>;
}
...
function OrderDialog({ accessToken, open, setOpen, detail, saving, onSubmit }: { accessToken: string; open: boolean; setOpen: (open: boolean) => void; detail: OrderDetail | null; saving: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const order = detail?.order;
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="detail-dialog">{!order ? <LoadingState /> : <><DialogHeader><DialogDescription>{String(order.order_number)}</DialogDescription><DialogTitle>{String(order.brand)} {String(order.model)}</DialogTitle></DialogHeader><div className="detail-summary"><div><span>Cliente</span><b>{String(order.customerName)}</b><small>{String(order.customerPhone)}</small></div><div><span>Falla reportada</span><b>{String(order.reported_issue)}</b></div><div><span>Fecha de ingreso</span><b>{formatDate(String(order.created_at))}</b></div></div>{detail?.photos.length ? <div className="photo-strip">{detail.photos.map((photo) => <ProtectedImage key={photo.id} accessToken={accessToken} src={`/api/files/${photo.objectKey}`} alt={photo.filename} />)}</div> : <div className="no-photos"><Camera /> Sin fotografías de recepción</div>}
