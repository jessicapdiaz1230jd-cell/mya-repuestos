    ...
    <main className="main-area">
      ...
      <div className="content-area">
        ...
        {loading ? <LoadingState /> : <>
          ...
          {view === "configuracion" && user.role === "admin" && <SettingsView config={config} logoVersion={logoVersion} saving={saving} users={managedUsers} externalAuth={Boolean(accessToken)} onLogo={updateLogo} onSave={saveConfig} onLoadUsers={loadUsers} onCreateUser={createUser} />}
        </>}
      </div>
    </main>
...
function SettingsView({ config, logoVersion, saving, users, externalAuth, onLogo, onSave, onLoadUsers, onCreateUser }: { config: AppConfig; logoVersion: number; saving: boolean; users: ManagedUser[]; externalAuth: boolean; onLogo: (event: FormEvent<HTMLFormElement>) => void; onSave: (config: AppConfig) => void; onLoadUsers: () => void; onCreateUser: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <section className="panel settings-workspace"><div className="settings-heading"><div><p className="eyebrow">CENTRO DE CONFIGURACIÓN</p><h2>Personaliza tu sistema sin programar</h2><p>Activa, oculta, renombra y organiza campos; cambia el diseño y administra accesos.</p></div><Button className="primary-action" disabled={saving} onClick={() => onSave(draft)}>{saving ? <RefreshCw className="spin" /> : <Save />} Guardar cambios</Button></div>
    <Tabs defaultValue="formulario" className="settings-tabs"><TabsList className="settings-tab-list"><TabsTrigger value="formulario"><ListChecks /> Formulario</TabsTrigger><TabsTrigger value="apariencia"><Palette /> Apariencia</TabsTrigger><TabsTrigger value="usuarios" onClick={() => { if (externalAuth) onLoadUsers(); }}><Users /> Usuarios</TabsTrigger></TabsList>
      ...
      <TabsContent value="usuarios">{externalAuth ? <div className="users-layout"><article><div className="settings-intro"><h3>Usuarios con acceso</h3><p>Cada persona entra con su propio correo y contraseña, sin usar ChatGPT.</p></div><div className="user-list">{users.length ? users.map((item) => <div key={item.id}><span className="user-avatar">{(item.name || item.email).slice(0, 2).toUpperCase()}</span><span><b>{item.name || "Usuario"}</b><small>{item.email}</small></span><em>{item.role === "admin" ? "Administrador" : item.role === "tecnico" ? "Técnico" : "Recepción"}</em></div>) : <p className="users-empty">Todavía no hay otros usuarios. Crea el primero con el formulario.</p>}</div></article><form className="create-user-card" onSubmit={onCreateUser}><UserPlus /><h3>Crear usuario</h3><p>La persona podrá entrar de inmediato con estos datos.</p><Label>Nombre</Label><Input name="name" required placeholder="Nombre completo" /><Label>Correo</Label><Input name="email" type="email" required placeholder="usuario@empresa.com" /><Label>Contraseña temporal</Label><Input name="password" type="password" minLength={8} required placeholder="Mínimo 8 caracteres" /><Label>Permiso</Label><select name="role" defaultValue="recepcion"><option value="recepcion">Recepción</option><option value="tecnico">Técnico</option><option value="admin">Administrador</option></select><Button className="primary-action" disabled={saving}><UserPlus /> Crear acceso</Button></form></div> : <div className="users-pending"><ShieldCheck /><h3>El módulo de usuarios está preparado</h3><p>Se habilitará automáticamente cuando conectemos la publicación independiente con Supabase. Mientras esta dirección siga siendo la versión privada de ChatGPT, el acceso continuará protegido por tu cuenta actual.</p></div>}</TabsContent>
    </Tabs>
  </section>;
}
